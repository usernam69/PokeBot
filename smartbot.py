...
# ---------------------------------------------------------------------------
# YOUR SETTINGS (loaded from environment variables - never hardcode credentials!)
# ---------------------------------------------------------------------------
BOT_USERNAME  = os.environ.get("BOT_USERNAME", "verycoolbot")
...
class SmartPlayer(Player):
    ...
    def choose_move(self, battle):
        try:
            ...
            # --- Terastallize decision ---
            use_tera = False
            try:
                can_tera = (
                    getattr(battle, "can_tera", False) or
                    getattr(battle, "can_terastallize", False)
                )
                if can_tera and opponent is not None and active is not None:
            ...
            for move in battle.available_moves:
                try:
                    ...
                    score = move.base_power / 200 if move.base_power > 0 else 0.01
                if score > best_score:
                    best_score = score
                    best_move  = move
            ...
            # --- Switch decision ---
            should_switch = False
            if opponent and active and battle.available_switches:
                try:
            ...
            print(f"[SmartPlayer] choose_move error: {e}")
